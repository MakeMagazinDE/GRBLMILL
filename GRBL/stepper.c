...
// already ran into limit switches to prevent damage -cm
  static volatile uint8_t limit_bits;

  limit_bits = LIMIT_PIN ^ LIMIT_INVMASK;

#ifdef X_LIMIT_PRESENT
  if (!(limit_bits & (1<<X_LIMIT_BIT)) && (out_bits & (1<<X_DIRECTION_BIT)))
    {
  ...
  if (!(limit_bits & (1<<Y_LIMIT_BIT)) && (out_bits & (1<<Y_DIRECTION_BIT)))
    {
  ...
  if (!(limit_bits & (1<<Z_LIMIT_BIT)) && (out_bits & (1<<Z_DIRECTION_BIT)))
    {
