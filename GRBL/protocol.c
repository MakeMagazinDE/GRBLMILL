...
static uint8_t emerg_stop; // Flag for Emergency Stop Button -cm
...
// Evaluate limit, aux & stop switch settings
  ...
  if (!(aux_bits & (1<<AUX_16_BIT)))
    {
    switchflags |= 8;
    }
  if (!(aux_bits & (1<<AUX_17_BIT)))
    {
    switchflags |= 16;
    }
  if (!(aux_bits & (1<<AUX_STOP_BIT)))
...
void protocol_init()
{
  // Print grbl initialization message if not in emergency stop mode
	if (!(emerg_stop)) {
